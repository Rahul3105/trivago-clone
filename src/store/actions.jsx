...
export const getAllHotel =
  ...
  (dispatch) => {
    ...
    return axios
      .get(
        `https://first-server24.herokuapp.com/MainData?q=${query}&_page=${currPage}`
      )
      .then((res) => {
...
export const priceFilter =
  (payload, query, filterType = "price") =>
  (dispatch) => {
    dispatch(hotelRequest());
    if (query === null) {
      axios
        .get(`https://first-server24.herokuapp.com/MainData`)
        .then((res) => {
          let { data } = res;
          let filtered;
          if (filterType === "price") {
      ...
      axios
        .get(`https://first-server24.herokuapp.com/MainData?q=${query}`)
        .then((res) => {
          let { data } = res;
          let filtered;
          if (filterType === "price") {
